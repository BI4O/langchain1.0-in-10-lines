...
from langchain.agents.middleware import (
    AgentState,
    AgentMiddleware, # new: import AgentMiddleware
    hook_config # new: import hook_config for jump functionality
)
from langgraph.store.memory import InMemoryStore

"""
when it comes to have CustomState, using class-style middleware is preferred.
You can define a class that inherits from AgentMiddleware and override the methods
"""
...
class ChatInfo(AgentMiddleware[CustomState]):
    state_schema = CustomState # must specify "state_schema", not "state"
    ...
    def after_model(self, state: CustomState, runtime: Runtime):
...
agent = create_agent(
    ...
    store=InMemoryStore(),
    middleware=[ChatInfo()]  # * new: add customized middleware as a instance
)
